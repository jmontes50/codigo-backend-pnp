...
CREATE TABLE IF NOT EXISTS `playas_2`.`t_vehiculos` (
  ...
  `t_vehiculoscol` VARCHAR(45) NULL,
  PRIMARY KEY (`veh_id`))
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `playas_2`.`t_playa` (
...
CREATE TABLE IF NOT EXISTS `playas_2`.`t_registro` (
  ...
  CONSTRAINT `fk_t_registro_t_vehiculos`
    FOREIGN KEY (`veh_id`)
    REFERENCES `playas_2`.`t_vehiculos` (`veh_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_t_registro_t_playa1`
    FOREIGN KEY (`playa_id`)
    REFERENCES `playas_2`.`t_playa` (`playa_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
